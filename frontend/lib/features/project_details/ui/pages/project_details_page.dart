...
class _ProjectDetailsPageState extends State<ProjectDetailsPage> {
  ...
  Future<void> _loadRelatedPosts() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          // Filter out the current project if it appears
          final filtered = newContent.where((item) {
            if (item is Project) return item.id != widget.projectId;
            return true;
          }).toList();
  ...
  Future<void> _loadData() async {
    ...
    try {
      final results = await Future.wait([
        _projectsRepository.getProject(widget.projectId),
        _commentsRepository.getComments(widget.projectId),
      ]);
  ...
  Future<void> _handleSendComment(String text) async {
    ...
    if (newComment != null && mounted) {
      setState(() {
        ...
        _project = _project!.copyWith(commentsCount: _project!.commentsCount + 1);
        _replyToId = null;
        _replyToUser = null;
      });
    }
  ...
  Future<void> _deleteComment(String id) async {
    ...
    if (success && mounted) {
      setState(() {
        ...
        _project = _project!.copyWith(commentsCount: (_project!.commentsCount - 1).clamp(0, double.infinity).toInt());
      });
    }
  }
