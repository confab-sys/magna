You are the Flutter frontend engineer for the Magna platform.

Goal:
When a user taps a ProjectCard in the Feed, navigate to a dedicated Project Details screen that shows the expanded project view, full project information, and the full comment thread, with like/comment/reply/delete working consistently.

STRICT RULES
- Do not hardcode colors, spacing, or typography. Use the Magna design tokens in:
  - lib/theme/colors.dart
  - lib/theme/typography.dart
  - lib/theme/spacing.dart
- Reuse existing components where possible. Avoid duplicating UI.
- Keep Feed fast: do not load full comment threads in the feed list.
- Use the existing API client + auth interceptors (core/network/api_client.dart) and your endpoints mapping.
- All actions (like, comment, reply, delete) must update UI state immediately (optimistic update) and then reconcile with API results.
- Reuse the same comment widgets/patterns already created for post details if possible.

------------------------------------------------------------
1) ROUTING / NAVIGATION
Add a new route for project details:

Route: /project/:projectId

Use go_router:
- In lib/app/router.dart, create a route that takes projectId from path params.
- From ProjectCard, on tap of the card (or tap of main content area), navigate:
  context.push('/project/${project.id}')

------------------------------------------------------------
2) FILE STRUCTURE
Create a new feature module (preferred):

lib/features/project_details/
  data/
    project_details_api.dart
  domain/
    project_details_models.dart   (optional; reuse existing Project/Comment models if they exist)
  ui/
    pages/project_details_page.dart
    widgets/project_header_section.dart
    widgets/project_meta_section.dart
    widgets/project_links_section.dart
    widgets/project_contributors_section.dart
    widgets/comment_thread.dart
    widgets/comment_tile.dart
    widgets/comment_composer.dart

If shared comment widgets already exist, reuse them instead of duplicating.

------------------------------------------------------------
3) PROJECT DETAILS PAGE UI
Create:
lib/features/project_details/ui/pages/project_details_page.dart

Layout requirements:
- Scaffold
- AppBar:
  - Back button
  - Title: “Project”
  - Optional more menu
- Body:
  A) Expanded Project view
  B) Full comment thread
- Bottom:
  - Fixed comment composer bar

Use:
- SafeArea
- Keyboard handling so composer stays visible above keyboard
- Scrollable body

------------------------------------------------------------
4) REUSE PROJECT CARD UI (NO DUPLICATION)
Modify your existing ProjectCard (or extract a shared ProjectView widget) so it can render in two modes:

ProjectCard(project: ..., mode: ProjectCardMode.feed)
ProjectCard(project: ..., mode: ProjectCardMode.details)

In details mode:
- Show full description (no truncation)
- Show full banner image if available
- Show full tech stack
- Show contributor information more clearly
- Keep like button functional
- Do NOT show truncated preview-only layout
- Do NOT show comment preview inside the card; full comments are below

In feed mode:
- Keep current compact card behavior

If you already implemented ProjectCard as a widget, add:
  final bool isDetailView;
and branch only where necessary.

------------------------------------------------------------
5) PROJECT DATA MODEL
Use the existing project schema and frontend model.

Project should support these fields:
- id
- title
- shortDescription
- description
- ownerName
- ownerAvatarUrl
- createdAt
- status
- visibility
- techStack : List<String>
- lookingForContributors : bool
- maxContributors : int?
- currentContributorsCount : int?
- repositoryUrl : String?
- liveDemoUrl : String?
- imageUrl : String?
- likesCount : int
- commentsCount : int

If some of these are not available yet from API, design the UI to support them cleanly.

------------------------------------------------------------
6) DATA LOADING REQUIREMENTS
When ProjectDetailsPage opens, load:
- Project by id (fresh copy)
- Comments for that project (top-level + replies)

Use your ApiClient and endpoints.
If endpoints are not defined yet, define them in core/network/endpoints.dart consistently.

Suggested endpoints (adapt to backend):
- GET /api/projects/:id
- GET /api/projects/:id/comments
- POST /api/projects/:id/like
- POST /api/projects/:id/comments
- POST /api/project-comments/:commentId/reply
- DELETE /api/project-comments/:commentId

Implement a small API wrapper in:
lib/features/project_details/data/project_details_api.dart

Expose:
- fetchProject(projectId)
- fetchComments(projectId)
- toggleLike(projectId, currentlyLiked)
- createComment(projectId, text)
- replyToComment(commentId, text)
- deleteComment(commentId)

------------------------------------------------------------
7) PROJECT DETAILS SCREEN CONTENT
The details page should present the project in this order:

A. HEADER SECTION
- Owner avatar
- Owner name
- Timestamp
- Status pill
- Visibility pill (optional)

B. PROJECT BANNER IMAGE
- Full width
- Rounded corners
- BoxFit.cover
- Show if imageUrl exists

C. TITLE + DESCRIPTION
- Large bold title
- Full description text (not truncated)

D. TECH STACK
- Show all chips in wrap layout
- Example: Flutter, Cloudflare, D1, Workers, Next.js

E. CONTRIBUTOR STATUS
If lookingForContributors == true:
- Show icon + “Looking for contributors”
- Show slots info if maxContributors exists
  Example: “2 / 5 slots filled”

F. LINKS SECTION
If repositoryUrl exists:
- Show Repo button/icon
If liveDemoUrl exists:
- Show Live Demo button/icon

G. PRIMARY ACTIONS
- View Project / Join Project / Apply to Contribute (depending on your flow)
- Save/Bookmark optional

H. ENGAGEMENT BAR
Below project info show:
- Like icon + likesCount
- Comment icon + commentsCount
- Share icon + text “Share”

Then below that:
I. FULL COMMENT THREAD

------------------------------------------------------------
8) COMMENT THREAD UI (WITH REPLIES)
Reuse the same comment architecture used for Post Details if possible.

A comment should support:
- id
- authorName
- authorAvatarUrl?
- createdAt
- text
- parentId? (null for top-level)
- replies: List<Comment>

Rendering rules:
- Show top-level comments in a list inside the page scroll
- Each comment tile shows:
  - avatar, name, timestamp
  - comment text
  - actions row: Reply, Delete (only if owned by current user), Like (optional)
- Replies are indented
- Support reply flow:
  - Tapping Reply sets composer into “replying to @username” mode
  - Sending posts a reply under that comment
  - Provide “cancel reply” action

Implementation hint:
- Use flat list and group by parentId, or render nested structure directly if backend already returns nested replies

------------------------------------------------------------
9) COMMENT COMPOSER (BOTTOM BAR)
Create or reuse:
lib/features/project_details/ui/widgets/comment_composer.dart

Features:
- TextField (multiline up to 4 lines)
- Send icon button
- Disabled send when empty
- Reply mode:
  - If replyingTo != null, show “Replying to @Name” + X to cancel

On send:
- If reply mode: call replyToComment(replyingTo.id, text)
- Else: call createComment(projectId, text)

Optimistic UI:
- Insert temporary comment immediately
- Replace with real API result after success

------------------------------------------------------------
10) STATE MANAGEMENT
Use whatever state approach is already in the app (Riverpod/Provider/setState).

Requirements:
- Loading state
- Error state with retry
- Optimistic updates for like/comment/reply/delete
- Pull-to-refresh optional

Like behavior:
- Toggle instantly in UI
- If API fails, revert and show snackbar/toast

Delete behavior:
- Remove comment instantly
- If API fails, restore and show error

------------------------------------------------------------
11) FEED INTEGRATION
From the Feed, ensure:
- Tapping a ProjectCard navigates to ProjectDetailsPage
- Returning back updates the feed item’s likes/comments count if possible

Recommended:
- Return updated project snapshot on pop
or
- Refresh only that one item locally

------------------------------------------------------------
12) DELIVERABLES CHECKLIST
- [ ] New route /project/:projectId works
- [ ] ProjectDetailsPage loads and displays expanded project
- [ ] Full description and full tech stack render correctly
- [ ] Repo/demo links render conditionally
- [ ] Contributor status section displays correctly
- [ ] Comments load and render with replies
- [ ] Reply flow works
- [ ] Delete comment works
- [ ] Like works from both feed and details
- [ ] No duplicated UI (ProjectCard reused with details mode)
- [ ] Uses design system tokens only

Build it clean, modular, and production-ready.

okay and then on project detail screen just below the first six comments make the feed to continue showing a mixture of related posts cards,jobs, projects with infinite scrolling